...
#Dataset
data = pd.read_csv('dataset/FINAL_MERGED_DATA_reimputed.parquet', engine="pyarrow")
        ...
        dbc.Col([
            ...
            dbc.Row([
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        id='heatmap-dropdown',
                        options=[{"label": "Select All", "value": "All"}] +[{'label': col, 'value': col} for col in corr_data.columns if col not in ['Country', 'Year', 'Age_Group', 'Gender']] ,
                    ...
                    dcc.Dropdown(
                        ...
                        options=[{'label': col, 'value': col} for col in corr_data.columns if col not in ['Country', 'Year', 'Age_Group', 'Gender']],
...
def register_callbacks_corr(app, cache):
    ...
    def create_sankey(trgt):
        ...
        def categorize_alcohol(x):
            if x <= alcohol_quartiles[0.25]: return "Low Alcohol"
            elif x <= alcohol_quartiles[0.75]: return "Moderate Alcohol"
            else: return "High Alcohol"

        def categorize_obesity(x):
            if x <= obesity_quartiles[0.25]: return "Low Obesity"
            elif x <= obesity_quartiles[0.75]: return "Moderate Obesity"
            else: return "High Obesity"

        def categorize_diabetes(x):
            if x <= diabetes_quartiles[0.25]: return "Low Diabetes"
            elif x <= diabetes_quartiles[0.75]: return "Moderate Diabetes"
            else: return "High Diabetes"

        def categorize_physical(x):
            if x <= physical_quartiles[0.25]: return "Low Physical Activity"
            elif x <= physical_quartiles[0.75]: return "Moderate Physical Activity"
            else: return "High Physical Activity"

        def categorize_trgt(x):
            if x <= trgt_quartiles[0.25]: return f"Low {trgt}"
            elif x <= trgt_quartiles[0.75]: return f"Moderate {trgt}"
            else: return f"High {trgt}"
        ...
        source_target_pairs_physical = df.groupby(["Physical Activity Level", "Region"]).size().reset_index(name="Count")
